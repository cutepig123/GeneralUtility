...
class Tuple3
{
  public:
    ...
    Tuple3(const Tuple3<T> &source){
      x = source.x;
      y = source.y;
      z = source.z;
    }

    Tuple3(const T* source){
      x = source[0];
      y = source[1];
      z = source[2];
    }
    ...
    Tuple3(const Tuple2<T> &source){
      x = source.x;
      y = source.y;
      z = 1.0;
    }

    Tuple3(const Tuple2<T> &source, const T Z){
      x = source.x;
      y = source.y;
      z = Z;
    }

    inline Tuple3 &operator = (const Tuple3<T> &right){
      x = right.x;
      y = right.y;
      z = right.z;
      return *this;
    ...
    inline void operator += (const Tuple3<T> &right){
      x+=right.x;
      y+=right.y;
      z+=right.z;
    }
    ...
    bool operator == (const Tuple3<T> &right)
    {
      return (x == right.x &&
              y == right.y &&
              z == right.z);
    ...
    inline T getLength()       { return sqrtf(x*x + y*y + z*z); }

    inline T getDotProduct(const Tuple3 &t)
    {
      return x*t.x + y*t.y + z*t.z;
    }

    inline Tuple3 operator ^(const Tuple3<T> &t)
    {
    ...
    inline Tuple3 &crossProduct(const Tuple3<T> &t1, const Tuple3<T> &t2)
    {
    ...
    inline T getDistance( Tuple3<T>& v2)
    {
