								 ...
								 IN PIRP pIrp)
{
	...
	{						// process request
		...
		case IOCTL_CREATEPROCESS:
		{
			KdPrint(("IOCTL_CREATEPROCESS\n"));
			
			if( sizeof(CreateProcessPar)!=cbin )
			{
				KdPrint(("size check fail\n"));
				status = STATUS_UNSUCCESSFUL;
				goto Exit;
			}
			//缓冲区方式IOCTL
			//显示输入缓冲区数据
 			CreateProcessPar* InputBuffer = (CreateProcessPar*)pIrp->AssociatedIrp.SystemBuffer;
			KdPrint(("acExePath %s InputBuffer->pWinExec %x\n", InputBuffer->acExePath, InputBuffer->pWinExec));
			RunProcess(InputBuffer->acExePath, InputBuffer->pWinExec);
			//操作输出缓冲区
			UCHAR* OutputBuffer = (UCHAR*)pIrp->AssociatedIrp.SystemBuffer;
			memset(OutputBuffer,0xAA,cbout);
			//设置实际操作输出缓冲区长度
