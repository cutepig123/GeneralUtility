...
void * FindRegAddr(bool w,char reg)
{
	switch(reg)
	{
	case 0:
...
void OpcError()
{
	...
	sprintf(err,"An unkown instruction [ %2x %2x %2x %2x %2x %2x ] was read at [ %4xh:%4xh ], easyVM only support 8086 instruction set in this version. easyVM will be terminated.",*(pc),*(pc+1),*(pc+2),*(pc+3),*(pc+4),*(pc+5),eCPU.cs,eCPU.ip);
	eVMerror(err);
}
